...
- **Håndtering af outliers**: Medianimputering blev brugt til at reducere påvirkningen af ekstreme værdier.
- **Manglende værdier**: Udfyldt med medianer eller estimeret ved hjælp af tidligere observationer.
- **Transformationer**: Variabler som eksempelvis `ugedag` og `årstid` blev oprettet for at forbedre modellens forklaringskraft.
...
Lasso Regression reducerede RMSE med ca. 10% sammenlignet med baseline og inkluderede seks centrale variabler hvor dem med højest vægt er `guld_menuer_sidste_møde`, `antal_tilskuere` og `luftfugtighed`.
