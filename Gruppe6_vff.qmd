...
format: 
  pdf:
   ...
   geometry:
      ...
      - bottom=30mm
      - heightrounded
abstract: "Her kan I skrive et abstract, som ikke bør fylde mere end en halv side. Bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, bla, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa, blaaaa....hej. Hej fra Martin" "abdi"
---
...
### Importer data til R
Datasættet blev indlæst i R fra flere kilder: 
1. **VIP-menu data** fra Excel-ark, 
2. **Superstats kampdata** vha. webscraping og 
3. **Meteorologiske observationer** fra DMI's API.

Data blev samlet ved hjælp af `inner_join` på kampdatoer, hvilket sikrede kun relevante observationer for analyse. Herefter blev irrelevante kolonner og rækker fjernet, og datastrukturen kontrolleret for konsistens.

### Output variabel
Den valgte **outputvariabel** er `guld_menu_stk`, der repræsenterer antallet af afhentede VIP-guldmenuer. Dette er målet for modellens forudsigelser.

### Forklarende variabler
De forklarende variabler inkluderede blandt andet:
- **Meteorologiske faktorer** som `vindretning`, `luftfugtighed`, `sigtbarhed` og `skydække`.
- **Kamprelaterede faktorer** som `antal_tilskuere` og `guldmenu_sidste_møde`.
- **Afledte faktorerr** som `ugedag`, `årstid` og `point_sidste_3_kampe`.

### Data pre-processing i R
Data blev forbehandlet for at sikre høj datakvalitet:
- **Håndtering af outliers**: Observation nr. 7 i Excel-filen blev identificeret som en outlier. Kampene i 2013 havde generelt en lavere afsætning af guldmenuer, men denne kamp mod FCM havde en usædvanligt høj afsætning på 1378 stk. Dette blev korrigeret ved at anvende medianimputering for at reducere påvirkningen af ekstreme værdier.
- **Manglende værdier**: Variabler som guld_menuer_sidste_møde og antal_tilskuere_sidste_møde indeholdt manglende værdier i tilfælde, hvor der ikke var data fra et tidligere møde. Disse blev erstattet med medianen for hele kolonnen for at sikre konsistens og reducere bias.
- **Transformationer**: Nye variabler som `ugedag` og `årstid` blev oprettet baseret på datovariablen for at styrke modellens forklaringskraft. Variablen ugedag gjorde det muligt at identificere mønstre i data, hvor mandagskampe havde en positiv effekt på afsætningen af guldmenuer sammenlignet med lørdagskampe. Ligeledes gav variablen årstid indsigt i, hvordan eksempelvis kampe spillet om sommeren påvirkede afsætningen af guldmenuer i forhold til kampe spillet om vinteren. Disse transformationer gav en dybere forståelse af datasættet og bidrog til en mere præcis model.

### Explorative analyse
Den eksplorative analyse inkluderede histogrammer, scatterplots og korrelationsmatrixer for at identificere mønstre og sammenhænge. Stærkt korrelerede variabler blev fjernet for at reducere multikollinearitet.

---

## Valg af metode til at teste model performance
For at evaluere modellens performance anvendte vi:
1. **Træning/test-split**: Datasættet blev opdelt i 2/3 træning og 1/3 test.
2. **Krydsvalidering**: 10-fold krydsvalidering blev brugt til at finde optimale parametre for Ridge og Lasso.
3. **Performance-målinger**: RMSE, MAE og R² blev anvendt til at vurdere præcision og forklaringsgrad.
...
Tre modeller blev sammenlignet:
1. **Baseline-model**: Forudsagde middelværdien uden forklarende variabler (RMSE = 147.49).
2. **Ridge Regression**: Reducerede RMSE til 125.67 ved at inkludere alle variabler.
3. **Lasso Regression**: Opnåede RMSE på 133.11 og forenklede modellen ved at udvælge de mest relevante variabler.

### Modeludvælgelse
Den endelige model blev valgt ud fra en balance mellem præcision og simplicitet. **Lasso Regression** blev valgt for dens evne til at reducere variabler uden væsentlig præcisionstab.

### Overordnet præcision i forudsigelse
Overordnet præcision i forudsigelse

Lasso Regression viste en forbedring i forudsigelsespræcisionen sammenlignet med baseline-modellen, hvor RMSE (Root Mean Squared Error) blev reduceret med cirka 10%. Modellen opnåede en forklaringsgrad (R²) på 18.4%, hvilket betyder, at modellen kan forklare 18.4% af variationen i efterspørgslen efter VIP-guldmenuer. Selvom dette resultat viser, at modellen har potentiale, er det også et tegn på, at væsentlige forklarende faktorer stadig mangler i datasættet.

Modellen identificerede seks centrale variabler, som bidrog til forudsigelsen. Blandt disse blev guld_menuer_sidste_møde vurderet som den vigtigste forklarende variabel med en væsentlig vægt (0.379), hvilket understreger betydningen af historisk data fra tidligere møder med samme modstander.

Variablen vindretning (0.042) bidrog også positivt og viste en lidt større vægt end antal_tilskuere (0.041), hvilket antyder, at vejrmæssige faktorer som vindretning kan have en marginalt større effekt på efterspørgslen end tilskuertal. Af de meteorologiske faktorer havde luftfugtighed en negativ vægt (-0.259), hvilket antyder, at højere luftfugtighed kan reducere efterspørgslen. sigtbarhed (0.001) og skydække (-0.067) havde mindre vægt i modellen, men kan stadig spille en rolle som supplerende forklarende variabler.

Denne brede kombination af historiske, demografiske og meteorologiske faktorer gav modellen en dybere forståelse af, hvad der driver efterspørgslen efter guldmenuer. Resultaterne viser potentialet for at anvende denne indsigt i praksis, men også behovet for yderligere data og optimering for at opnå en endnu højere præcision og forklaringskraft.
...
Modellen opnåede en **RMSE på 133.11** og en forklaringsgrad (R²) på 18.4%. Hvilket indikerer, at modellen kun forklarer 18.4% af variabiliteten i data. Resultaterne antyder, at væsentlige forklarende faktorer mangler, og modellen bør forbedres med yderligere data.

### Vurdering af bidraget til løsningen af forretningsproblemet
Modellen giver indsigt i faktorer som ugedage og modstanderens historik, der påvirker efterspørgslen. F.eks. viser data, at bundklubben Vejle trækker mange guldmenuer, hvilket kan skyldes sociale relationer eller arrangementer. Det er dog også med en vis statistisk usikkerhed, da de kun har mødt  Vejle fire gange i Superligaen 2013-2023.

### Udrulning af anbefalingerne
For at gøre modellen mere anvendelig anbefales:
1. **Indsamling af yderligere data** som netværksaktiviteter og events.
2. **Analyse af sociale faktorer** som mønstre i VIP-gæsters adfærd.
3. **Løbende opdatering af modellen** med nye data for at forbedre præcisionen.
4. **Lave en ny variabel** som indebærer de forskellige sponsorer, for at få indblik i deres brugeradfærd af guldmenuer.

# Konklusion
Lasso Regression-modellen viser potentiale for at forudsige efterspørgslen efter VIP-guldmenuer. Dog kræves yderligere variabler og løbende opdateringer for at opnå højere præcision og forretningsværdi. Konklusionen er, at modellen endnu ikke er tilstrækkelig præcis til praktisk anvendelse, sandsynligvis på grund af manglende data. Der er stadig mennesker hos Viborg F.F. (Palle), der skal vurdere hvor mange der kommer og afhenter guldmenuer til deres hjemmekampe.
...
# ============================================================================== 
# Visualisering af residualer for Lasso Regression
# ============================================================================== 
# Vi visualiserer residualerne for at analysere, hvordan de fordeler sig
# i forhold til de forudsagte værdier.

ggplot(lasso_residuals_df, aes(x = Predicted, y = Residuals)) +
  
  # Plot residualerne som punkter
  geom_point(color = "blue", alpha = 0.6) +
  # `geom_point` viser hver residual som et punkt i plottet.
  # Farven blå (`color = "blue"`) bruges for klarhed, og gennemsigtigheden (`alpha = 0.6`)
  # reducerer overlap mellem punkter, så mønstre bliver lettere at se.
  
  # Tilføj en horisontal linje ved y = 0
  geom_hline(yintercept = 0, linetype = "dashed", color = "red", size = 1) +
  # En stiplet, rød linje (`linetype = "dashed", color = "red"`) markerer nulpunktet.
  # Residualerne bør ideelt set være tilfældigt fordelt omkring denne linje.
  
  # Tilføj labels og titel
  labs(
    title = "Residual Plot for Lasso-Regression",  # Titel, der beskriver plottet.
    x = "Forudsagte værdier",                      # Beskrivelse af x-aksen.
    y = "Residuals (Fejl)"                         # Beskrivelse af y-aksen.
  ) +
  
  # Anvend et minimalistisk tema for bedre læsbarhed
  theme_minimal()
# `theme_minimal()` fjerner overflødige grafikelementer, hvilket gør plottet mere læsbart.
